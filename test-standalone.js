          ...
          inputSchema: {
            ...
            properties: {
              ...
              workFolder: {
                type: "string",
                description:
                  "Mandatory when using file operations or referencing any file. The working directory for the Claude CLI execution. Must be an absolute path.",
              },
            },
            required: ["prompt"],
          },
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (args, call) => {
      ...
      if (
        ...
        throw new McpError(
          ...
          'Missing or invalid required parameter: prompt (must be an object with a string "prompt" property) for claude_code tool',
        );
      }
      let effectiveCwd = homedir();
      if (
        toolArguments.workFolder &&
        typeof toolArguments.workFolder === "string"
      ) {
        const resolvedCwd = pathResolve(toolArguments.workFolder);
        debugLog(
          `[Debug] Specified workFolder: ${toolArguments.workFolder}, Resolved to: ${resolvedCwd}`,
        );
      ...
      try {
        ...
        debugLog(
          `[Debug] Invoking Claude CLI: ${this.claudeCliPath} ${claudeProcessArgs.join(" ")}`,
        );
...
Stdout: ${error.stdout}`;
        ...
        if (
          ...
          throw new McpError(
            ErrorCode.InternalError,
            `Claude CLI command timed out after ${executionTimeoutMs / 1e3}s. Details: ${errorMessage}`,
          );
        }
        throw new McpError(
          ErrorCode.InternalError,
...
export { ClaudeCodeServer, debugLog, findClaudeCli, spawnAsync };
